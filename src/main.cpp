...
void modbus_listen(AppContext* ctx) {
    ...
    constexpr size_t MODBUS_RTU_MAX_ADU_LENGTH = 260;

    auto serial = ctx->serial;
    std::vector<uint8_t> buf(MODBUS_RTU_MAX_ADU_LENGTH);
    std::array<uint8_t, MODBUS_RTU_MAX_ADU_LENGTH> chunk{};
    ...
    while (ctx->is_running.load(std::memory_order_relaxed)) {
        try {
            ...
            int n = serial->call(serial_read, chunk.data(), 1, 10);
            if (n > 0) {
                buf.clear();
                buf.push_back(chunk[0]);
                while (true) {
                    int more = serial->call(serial_read, chunk.data(), chunk.size(), INTER_BYTE_TIMEOUT_MS);
                    if (more > 0) {
                        buf.insert(buf.end(), chunk.begin(), chunk.begin() + more);
                        if (buf.size() >= MODBUS_RTU_MAX_ADU_LENGTH) break;
                    } else {
                        break;
                    }
                }
            ...
            print_error(ErrorType::SYSTEM, e.what());
        }
 ...
 * input formats:
 * - req <frame>
 * - req<expected_response_length> <frame>
...
void process_line(AppContext& ctx, const std::string& line) {
    ...
    try {
        if (!is_any(action, Action::WB, Action::RB, Action::WR, Action::RR, Action::RIB, Action::RIR)) {
            bool is_req = (action == Action::REQUEST);
            int expected_len = -1;

            if (!is_req && action.starts_with(Action::REQUEST)) {
                is_req = true;
                expected_len = parse_number<uint32_t>(action.substr(Action::REQUEST.length()));
                if (expected_len <= 0) throw std::invalid_argument("invalid expected response length");
            }

            if (is_req) {
                auto values = read_payload<uint8_t>(iss);
                if (values.size() < 2) {
                    throw std::invalid_argument("payload too short, requires at least slave_id and function_code");
                }
                if (expected_len == -1) {
                    expected_len = get_response_len(std::span{values}.subspan(1)) - 1;
                }
                uint8_t slave_id = values[0];
                uint8_t function_code = values[1];
                values.erase(values.begin(), values.begin() + 2);
                modbus->invoke(nmbs_set_destination_rtu_address, slave_id);
                modbus->call(nmbs_send_raw_pdu, function_code, values.data(), values.size());
                modbus->call(nmbs_receive_raw_pdu_response, nullptr, expected_len);
                const auto* nmbs = modbus->ctx();
                if (nmbs->msg.buf_idx >= 2) {
                    print(format_payload(std::span{nmbs->msg.buf, static_cast<std::size_t>(nmbs->msg.buf_idx - 2)}));
                } else {
                    print_error(ErrorType::MODBUS, "invalid or empty response");
                }
                return;
            }
        ...
        auto slave_id = parse_number<uint8_t>(_slave_id);
        auto address = parse_number<uint16_t>(_address);
        uint16_t count = 1;

        if (is_any(action, Action::RB, Action::RR, Action::RIB, Action::RIR)) {
            count = read_number<uint32_t>(iss);
            if (count <= 0) {
        ...
        if (action == Action::WB) {
            auto payload = read_payload<uint8_t>(iss);
            count = payload.size();
            if (count == 1) {
        ...
        } else if (action == Action::RB) {
            std::vector<uint8_t> payload(count);
            modbus->call(nmbs_read_coils, address, count, payload.data());
            print(format_payload(payload, count));

        } else if (action == Action::WR) {
            auto payload = read_payload<uint16_t>(iss);
            count = payload.size();
            if (count == 1) {
...
int main(int argc, char* argv[]) {
    ...
    serial.call(serial_open_advanced, config.device.c_str(),
                static_cast<uint32_t>(config.baud_rate),
